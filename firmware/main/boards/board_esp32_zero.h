// board_esp32_zero.h — pin map for the "ESP32 Zero" board target.
// Included only from board_config.h when BOARD_ESP32_ZERO is defined there — don't include
...
#define BOARD_I2C_SDA_GPIO   GPIO_NUM_9
...
#define BOARD_SPI_SCLK_GPIO  GPIO_NUM_1
#define BOARD_SPI_MOSI_GPIO  GPIO_NUM_3
#define BOARD_SPI_MISO_GPIO  GPIO_NUM_2

// All SPI chip-select lines on the board (sensors + the onboard display). Index -1 is the
// display's CS; bus_spi leaves it alone (esp_lcd drives it). Set an entry to -1 if not wired.
#define BOARD_SPI_CS_COUNT     5
#define BOARD_SPI_CS_GPIOS     { GPIO_NUM_12, GPIO_NUM_13, GPIO_NUM_6, GPIO_NUM_7, GPIO_NUM_10 }
#define BOARD_DISPLAY_CS_INDEX -1    // CS index reserved for the onboard display (-1 = none)

// ---- User button (BOOT) — advances display pages in paged mode ----
#define BOARD_BUTTON_GPIO    -1   // active-low, onboard pull-up. -1 = none

// ---- UART (auxiliary port; UART0 is the console) ----
#define BOARD_UART_PORT      1
#define BOARD_UART_TX_GPIO   GPIO_NUM_4
#define BOARD_UART_RX_GPIO   GPIO_NUM_5
...
#define BOARD_LEGO_TX_GPIO    43
#define BOARD_LEGO_RX_GPIO    44
...
#define BOARD_TFT_MISO_GPIO  -1   // shared-bus MISO (the display itself never reads)
...
// BOARD_TFT_BL_I2C_* macros must exist even on a board that doesn't use it. Actually using
// this bus at runtime additionally requires BOARD_TFT_BL_I2C_ADDR to be defined (it isn't
// here), so these disabled values (-1 SDA/SCL) are never dereferenced — see display.c's and
// imu.c's `#if defined(BOARD_TFT_BL_I2C_ADDR)` guards around every call site.
